...
- [ ] `/support-workers` shows table with avatar, name, location, availability, specializations
- [ ] Worker count shown (e.g. "8 of 8 workers")

**Name filter**
- [ ] Type "Olivia" → only Olivia Williams visible; clear → all return

**Location filter**
- [ ] Type a suburb (e.g. "Sydney") — autocomplete suggestions appear
- [ ] Select a suggestion — radius slider appears; workers outside radius disappear
- [ ] Drag slider to 100 km — more workers appear

**Availability day filter**
- [ ] Select "Mon" — only workers available Mondays shown
- [ ] Select "Mon" + "Wed" — must be available on both

**Specialization filter**
- [ ] Click "Elderly Care" chip — only workers with that specialization shown
- [ ] Click multiple chips — workers must have ALL selected

**Filter badge + clear**
- [ ] Apply two filters — badge shows count; "Clear all" resets everything
...
- [ ] Profile shows name, location, bio, availability, specialization chips
- [ ] Police Check and WWCC chips visible if the worker has them
- [ ] "Message" button creates or opens a conversation → navigates to `/messages/:id`
